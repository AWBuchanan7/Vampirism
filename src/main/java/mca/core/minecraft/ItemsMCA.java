...
public final class ItemsMCA {
    ...
    public static final Item ROSE_GOLD_DUST = new Item();
    ...
    public static void register(RegistryEvent.Register<Item> event) {
        for (Field f : ItemsMCA.class.getFields()) {
            try {
                ...
                VampirismMod.getLog().error("Error while registering items: ", e);
            }
        }
    }

    public static void assignCreativeTabs() {
        ITEMS.stream().forEach(i -> i.setCreativeTab(VampirismMod.creativeTab));
    }
