    ...
    public static class CareerRequestHandler implements IMessageHandler<CareerRequest, IMessage> {
        ...
        public IMessage onMessage(CareerRequest message, MessageContext ctx) {
            ...
            try {
            	...
            	VampirismMod.getLog().error("UUID provided in career request does not match an MCA villager!: " + message.entityUUID.toString());
                return null;
            } catch (NullPointerException ignored) {
            	VampirismMod.getLog().error("UUID provided in career request does not match a loaded MCA villager!: " + message.entityUUID.toString());
                return null;
            }

            if (careerId == -255) {
            	VampirismMod.getLog().error("Career ID wasn't assigned for UUID: " + message.entityUUID);
                return null;
            }
    ...
    public static class SetProfessionHandler implements IMessageHandler<SetProfession, IMessage> {
        ...
        public IMessage onMessage(SetProfession message, MessageContext ctx) {
            ...
            if (entity.isPresent()) {
            	...
            	VampirismMod.getLog().error("Entity not found on career set!: " + message.targetUUID.toString());
                return null;
            }
