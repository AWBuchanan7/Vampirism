...
public class ClientMessageQueue {
    ...
    private static void handle(IMessage msg) {
        ...
        else VampirismMod.getLog().error("Unexpected message in queue:" + msg.getClass().getName());
    }
    ...
    private static void handleCareerId(NetMCA.CareerResponse msg) {
        ...
        try {
        	...
        	VampirismMod.getLog().error("Failed to cast entity to villager on career ID update.");
        } catch (Exception e) {
        	VampirismMod.getLog().error("Failed to set career ID on villager!", e);
        }
    }
    ...
    private static Optional<EntityVillagerMCA> getVillagerByUUID(World world, UUID uuid) {
        try {
        	...
        	VampirismMod.getLog().error("Failed to cast entity with UUID " + uuid.toString() + " to a villager!");
        }
