...
public class API {
    ...
    public static void init() {
        ...
        for (Gift gift : gifts) {
            if (!gift.exists()) {
            	VampirismMod.getLog().warn("Could not find gift item or block in registry: " + gift.getName());
            } else {
    ...
    public static void addButtons(String guiKey, @Nullable EntityVillagerMCA villager, EntityPlayer player, GuiScreen screen) {
        ...
        for (APIButton b : buttonMap.get(guiKey)) {
            ...
            if (villager == null && b.getConstraints().size() > 0) {
            	VampirismMod.getLog().error("No villager provided for list of buttons with constraints! Button ID:" + b.getIdentifier());
                continue;
            }
