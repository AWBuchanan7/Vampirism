...
public class ItemBaby extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (world.isRemote && getBabyName(stack).equals(""))
            player.openGui(VampirismMod.getInstance(), Constants.GUI_ID_NAMEBABY, player.world, player.getEntityId(), 0, 0);
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        Localizer loc = VampirismMod.getLocalizer();
        ...
        if (stack.hasTagCompound()) {
            ...
            String ownerName = nbt.getUniqueId("ownerUUID").equals(player.getUniqueID()) ? VampirismMod.getLocalizer().localize("label.you") : nbt.getString("ownerName");

            if (getBabyName(stack).equals(""))
                tooltip.add(textColor + loc.localize("gui.label.name") + " " + Constants.Format.RESET + VampirismMod.getLocalizer().localize("label.unnamed"));
            else
    ...
    private boolean isReadyToGrowUp(ItemStack itemStack) {
        return itemStack.getTagCompound().getInteger("age") >= VampirismMod.getConfig().babyGrowUpTime;
    }
